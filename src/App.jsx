...
const TRAIT_WEIGHTS={QB:{"Accuracy":.25,"Decision Making":.22,"Pocket Presence":.18,"Arm Strength":.15,"Leadership":.09,"Pre-Snap Diagnosis":.08,"Mobility":.03},RB:{"Vision":.22,"Elusiveness":.20,"Contact Balance":.18,"Power":.15,"Speed":.10,"Pass Catching":.09,"Pass Protection":.06},WR:{"Route Running":.25,"Separation":.25,"Hands":.18,"YAC Ability":.16,"Release Package":.08,"Contested Catches":.05,"Speed":.03},TE:{"Receiving":.20,"Blocking":.20,"Route Running":.18,"Hands":.16,"Athleticism":.14,"Speed":.12},OT:{"Pass Protection":.25,"Footwork":.20,"Anchor":.18,"Run Blocking":.15,"Athleticism":.12,"Strength":.10},IOL:{"Run Blocking":.22,"Pass Protection":.22,"Strength":.18,"Pulling":.14,"Anchor":.13,"Versatility":.11},EDGE:{"Pass Rush":.24,"First Step":.20,"Bend":.18,"Power":.15,"Motor":.13,"Run Defense":.10},DL:{"Run Defense":.22,"Pass Rush":.20,"First Step":.18,"Hand Usage":.16,"Motor":.14,"Strength":.10},LB:{"Instincts":.22,"Tackling":.20,"Athleticism":.15,"Coverage":.12,"Pass Rush":.09,"Range":.08,"Pre-Snap Diagnosis":.07,"Block Shedding":.07},CB:{"Man Coverage":.24,"Ball Skills":.20,"Speed":.18,"Press":.15,"Zone Coverage":.13,"Nickel":.10},S:{"Man Coverage":.10,"Range":.25,"Ball Skills":.20,"Tackling":.20,"Speed":.15,"Nickel":.10},"K/P":{"Accuracy":.25,"Leg Strength":.22,"Consistency":.20,"Clutch":.13,"Directional Control":.12,"Hang Time":.08}};
...
const DRAFT_ORDER=[{pick:1,team:"Raiders"},{pick:2,team:"Jets"},{pick:3,team:"Cardinals"},{pick:4,team:"Titans"},{pick:5,team:"Giants"},{pick:6,team:"Browns"},{pick:7,team:"Commanders"},{pick:8,team:"Saints"},{pick:9,team:"Chiefs"},{pick:10,team:"Bengals"},{pick:11,team:"Dolphins"},{pick:12,team:"Cowboys"},{pick:13,team:"Rams"},{pick:14,team:"Raiders"},{pick:15,team:"Buccaneers"},{pick:16,team:"Jets"},{pick:17,team:"Lions"},{pick:18,team:"Vikings"},{pick:19,team:"Panthers"},{pick:20,team:"Cowboys"},{pick:21,team:"Steelers"},{pick:22,team:"Chargers"},{pick:23,team:"Eagles"},{pick:24,team:"Browns"},{pick:25,team:"Bears"},{pick:26,team:"Bills"},{pick:27,team:"49ers"},{pick:28,team:"Texans"},{pick:29,team:"Chiefs"},{pick:30,team:"Broncos"},{pick:31,team:"Patriots"},{pick:32,team:"Seahawks"}];
...
function DraftBoard({user,onSignOut,isGuest,onRequireAuth,onOpenGuide}){
  ...
  const finishRanking=useCallback((pos)=>{
    ...
    else{setBoardInitPos(pos);setPhase("board");}
  ...
  const sharePositionTop10=useCallback(async(pos)=>{
    ...
    const drawRadar=(cx,cy,r,traitNames,values,color)=>{
      ...
      const rK=1.8,rCurve=v=>Math.pow(v/100,rK)*100,rFLOOR=rCurve(40);
      const rGrid=[50,60,70,80,90,100].map(lv=>Math.max(0,(rCurve(lv)-rFLOOR)/(100-rFLOOR)));
      ctx.strokeStyle='#e0e0e0';ctx.lineWidth=0.5;
      rGrid.forEach((lv,li)=>{
        ctx.beginPath();
        angles.forEach((a,i)=>{const x=cx+r*lv*Math.cos(a),y=cy+r*lv*Math.sin(a);i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});
        ctx.closePath();if(li===2){ctx.strokeStyle='#d4d4d4';ctx.lineWidth=0.8;}else{ctx.strokeStyle='#e0e0e0';ctx.lineWidth=0.5;}ctx.stroke();
      });
      angles.forEach(a=>{ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+r*Math.cos(a),cy+r*Math.sin(a));ctx.strokeStyle='#e0e0e0';ctx.lineWidth=0.5;ctx.stroke();});
      const pts=angles.map((a,i)=>{const raw=values[i]||50;const v=Math.max(0,(rCurve(raw)-rFLOOR)/(100-rFLOOR));return[cx+r*v*Math.cos(a),cy+r*v*Math.sin(a)];});
  ...
  if(phase==="home"||phase==="pick-position"){
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))",gap:12}}>{POSITION_GROUPS.map(pos=>{const ct=(byPos[pos]||[]).length;const done=rankedGroups.has(pos);const partial=!!partialProgress[pos];const reviewed=traitReviewedGroups.has(pos);const c=POS_COLORS[pos];
      ...
      return(<div key={pos} style={{background:"#fff",border:"1px solid #e5e5e5",borderRadius:12,padding:"18px 20px",position:"relative"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          ...
          {partial&&!done&&<>
            <button onClick={()=>{setBoardInitPos(pos);setPhase("board");}} style={{fontFamily:sans,fontSize:12,fontWeight:700,padding:"7px 18px",background:`${c}11`,color:c,border:`1px solid ${c}33`,borderRadius:99,cursor:"pointer"}}>✓ rankings</button>
          ...
          {done&&<>
            <button onClick={()=>{setBoardInitPos(pos);setPhase("board");}} style={{fontFamily:sans,fontSize:12,fontWeight:700,padding:"7px 18px",background:reviewed?`${c}11`:"#171717",color:reviewed?c:"#faf9f6",border:reviewed?`1px solid ${c}33`:"none",borderRadius:99,cursor:"pointer"}}>{reviewed?"✓ rankings":"rankings"}</button>
          </>}
        </div>
      </div>);})}</div>
    ...
    <button onClick={()=>{setShowRound1Prediction(true);trackEvent(user?.id,'round1_prediction_cta_click',{guest:!user});}} style={{width:"100%",padding:"14px 20px",marginTop:32,background:"linear-gradient(135deg,#1e293b,#0f172a)",color:"#94a3b8",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,cursor:"pointer",marginBottom:24,display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:sans,fontSize:14,fontWeight:600}}>
      🔮 simulate round 1
    </button>
