    ...
    <>
      ...
      <div style={{position:"fixed",top:0,right:0,bottom:0,width:Math.min(460,window.innerWidth-24),background:"#faf9f6",zIndex:201,boxShadow:"-8px 0 32px rgba(0,0,0,0.12)",transform:isOpen?"translateX(0)":"translateX(100%)",transition:"transform 0.3s cubic-bezier(0.16,1,0.3,1)",display:"flex",flexDirection:"column",overflowY:"auto",overflowX:"hidden"}}>
        ...
        <div style={{padding:"0 24px 16px"}}>
          {(()=>{const ceilVal=traits[player.id]?.__ceiling??50;const ceilColor=ceilVal>50?`rgb(${Math.round(234-(234-22)*(ceilVal-50)/50)},${Math.round(179-(179-163)*(ceilVal-50)/50)},${Math.round(8+(41-8)*(ceilVal-50)/50)})`:ceilVal<50?`rgb(${Math.round(234+(220-234)*(50-ceilVal)/50)},${Math.round(179-(179-38)*(50-ceilVal)/50)},${Math.round(8+(38-8)*(50-ceilVal)/50)})`:"#a3a3a3";return(
            <div style={{marginBottom:20,paddingBottom:16,borderBottom:"1px solid #f0f0f0"}}>
              <div style={{marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                  <span style={{fontFamily:mono,fontSize:12,fontWeight:700,color:"#b8860b",letterSpacing:1}}>⭐ Ceiling</span>
                  <span style={{fontFamily:font,fontSize:16,fontWeight:900,color:ceilColor}}>{ceilVal}</span>
                </div>
                <div style={{position:"relative",height:28,display:"flex",alignItems:"center"}}>
                  <div style={{position:"absolute",left:0,right:0,height:8,background:"#f5f0e0",borderRadius:4}}/>
                  <div style={{position:"absolute",left:0,height:8,width:`${ceilVal}%`,background:"linear-gradient(90deg, #d97706, #eab308)",borderRadius:4}}/>
                  <div style={{position:"absolute",left:`${ceilVal}%`,transform:"translateX(-50%)",fontSize:20,lineHeight:1,pointerEvents:"none",zIndex:3,filter:"drop-shadow(0 1px 2px rgba(0,0,0,.2))"}}>⭐</div>
                  <input type="range" min="0" max="100" value={ceilVal}
                    onChange={e=>setTraits(prev=>({...prev,[player.id]:{...prev[player.id],__ceiling:parseInt(e.target.value)}}))}
                    style={{position:"absolute",left:0,width:"100%",height:28,background:"transparent",cursor:"pointer",zIndex:4,opacity:0,margin:0}}/>
                </div>
              </div>
            </div>
          );})()}
...
function DraftBoard({user,onSignOut}){
  ...
  const finishRanking=useCallback((pos,r)=>{const rts=r||ratings;const sorted=[...(byPos[pos]||[])].sort((a,b)=>(rts[b.id]||1500)-(rts[a.id]||1500));const nt={...traits};sorted.forEach((p,i)=>{if(!nt[p.id])nt[p.id]={};if(nt[p.id].__ceiling==null)nt[p.id].__ceiling=50;const base=Math.round(85-(i/Math.max(sorted.length-1,1))*50);(POSITION_TRAITS[p.gpos||p.pos]||POSITION_TRAITS[p.pos]||[]).forEach(t=>{nt[p.id][t]=Math.max(10,Math.min(99,base+Math.floor(Math.random()*16)-8));});});setTraits(nt);setRankedGroups(prev=>new Set([...prev,pos]));setPartialProgress(prev=>{const n={...prev};delete n[pos];return n;});setPhase("pick-position");},[ratings,byPos,traits]);
  ...
  }const p=PROSPECTS.find(x=>x.id===id);const pos=p?(p.gpos||p.pos):"QB";const weights=TRAIT_WEIGHTS[pos]||TRAIT_WEIGHTS["QB"];const posTraits=POSITION_TRAITS[pos]||[];let totalW=0,totalV=0;posTraits.forEach(trait=>{const w=weights[trait]||1/posTraits.length;const v=t[trait]||50;totalW+=w;totalV+=v*w;});const base=totalW>0?totalV/totalW:50;const ceil=t.__ceiling;const ceilBoost=ceil!=null?((ceil-50)/50)*15:0;return Math.max(1,Math.min(99,Math.round(base+ceilBoost)));},[traits]);
  ...
  // === BIG BOARD ===
  if(phase==="board")return(<div><SaveBar/><div style={{paddingTop:32}}><BoardView getBoard={getBoard} getGrade={getGrade} rankedGroups={rankedGroups} setPhase={setPhase} setSelectedPlayer={setSelectedPlayer} setActivePos={setActivePos} traits={traits} compareList={compareList} setCompareList={setCompareList} setProfilePlayer={setProfilePlayer} setShowMockDraft={setShowMockDraft} communityBoard={communityBoard} setCommunityBoard={setCommunityBoard}/></div>{profilePlayer&&<PlayerProfile player={profilePlayer} traits={traits} setTraits={setTraits} notes={notes} setNotes={setNotes} allProspects={PROSPECTS} getGrade={getGrade} onClose={()=>setProfilePlayer(null)} onSelectPlayer={setProfilePlayer} consensus={CONSENSUS} ratings={ratings}/>}</div>);
...
function BoardView({getBoard,getGrade,rankedGroups,setPhase,setSelectedPlayer,setActivePos,traits,compareList,setCompareList,setProfilePlayer,setShowMockDraft,communityBoard,setCommunityBoard}){
