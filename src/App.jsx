...
import { POSITION_TRAITS, TRAIT_EMOJI, TRAIT_ABBREV, TRAIT_SHORT, TRAIT_WEIGHTS, TRAIT_TEACHABILITY, POSITION_GROUPS, POS_EMOJI, POS_COLORS, posGroup } from "./positions.js";
...
function DraftBoard({user,onSignOut,isGuest,onRequireAuth,onOpenGuide,gmQuizMockLaunch,onClearGmQuizMock,adminOverrides}){
    ...
    saveTimer.current=setTimeout(async()=>{
      ...
      try{
        ...
        if(rankedGroups.size>0){
          try{
            const communityData={};
            PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=posGroup(g);return rankedGroups.has(group);}).forEach(p=>{communityData[p.id]=ratings[p.id]||1500;});
  ...
  const byPos=useMemo(()=>{const m={};const seen=new Set();PROSPECTS.forEach(p=>{if(seen.has(p.id))return;seen.add(p.id);const g=p.gpos||p.pos;const group=posGroup(g);if(!m[group])m[group]=[];m[group].push(p);});return m;},[]);
        ...
        {explorerMode==="scheme-fit"&&(()=>{
          ...
          if(!sfPosFilter){
            ...
            const agg=SF_GROUPS.map(pos=>{
              const atPos=PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=posGroup(g);return group===pos&&group!=="K/P";});
          ...
          const atPos=PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=posGroup(g);return group===sfPosFilter&&group!=="K/P";});
  ...
  if(phase==="home"||phase==="pick-position"){
    ...
    let filteredBoard=boardFilter.size>0?showBoard.filter(p=>{const g=posGroup(p.gpos||p.pos);return boardFilter.has(g);}):showBoard;
      ...
      {boardTab==="my"&&userBoard.length===0?(
        ...
        <div style={boardShowAll?{}:{maxHeight:480,overflowY:"auto"}}>
          {(boardShowAll?filteredBoard:filteredBoard.slice(0,100)).map((p,i)=>{
            ...
            const rank=i+1;
            const gpos=posGroup(p.gpos||p.pos);
