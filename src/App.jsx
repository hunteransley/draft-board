...
function DraftBoard({user,onSignOut,isGuest,onRequireAuth,onOpenGuide,gmQuizMockLaunch,onClearGmQuizMock,adminOverrides}){
    ...
    saveTimer.current=setTimeout(async()=>{
      ...
      try{
        ...
        if(rankedGroups.size>0){
          try{
            const communityData={};
            PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=(g==="K"||g==="P"||g==="LS")?"K/P":(g==="C"||g==="OG")?"IOL":g;return rankedGroups.has(group);}).forEach(p=>{communityData[p.id]=ratings[p.id]||1500;});
  ...
  const byPos=useMemo(()=>{const m={};const seen=new Set();PROSPECTS.forEach(p=>{if(seen.has(p.id))return;seen.add(p.id);const g=p.gpos||p.pos;const group=(g==="K"||g==="P"||g==="LS")?"K/P":(g==="C"||g==="OG")?"IOL":g;if(!m[group])m[group]=[];m[group].push(p);});return m;},[]);
  ...
  const board=useMemo(()=>PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=(g==="K"||g==="P"||g==="LS")?"K/P":(g==="C"||g==="OG")?"IOL":g;const hasTraitEdits=traits[p.id]&&Object.keys(traits[p.id]).length>0;return rankedGroups.has(group)||hasTraitEdits;}).sort((a,b)=>{const d=getGrade(b.id)-getGrade(a.id);return d!==0?d:(ratings[b.id]||1500)-(ratings[a.id]||1500);}),[rankedGroups,getGrade,ratings,traits]);
        ...
        {explorerMode==="scheme-fit"&&(()=>{
          ...
          if(!sfPosFilter){
            ...
            const agg=SF_GROUPS.map(pos=>{
              const atPos=PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=(g==="C"||g==="OG")?"IOL":g;return group===pos&&group!=="K/P";});
          ...
          const atPos=PROSPECTS.filter(p=>{const g=p.gpos||p.pos;const group=(g==="C"||g==="OG")?"IOL":g;return group===sfPosFilter&&group!=="K/P";});
  ...
  if(phase==="home"||phase==="pick-position"){
    ...
    let filteredBoard=boardFilter.size>0?showBoard.filter(p=>{const g=(p.gpos||p.pos)==="K"||(p.gpos||p.pos)==="P"||(p.gpos||p.pos)==="LS"?"K/P":((p.gpos||p.pos)==="C"||(p.gpos||p.pos)==="OG")?"IOL":(p.gpos||p.pos);return boardFilter.has(g);}):showBoard;
      ...
      {boardTab==="my"&&userBoard.length===0?(
        ...
        <div style={boardShowAll?{}:{maxHeight:480,overflowY:"auto"}}>
          {(boardShowAll?filteredBoard:filteredBoard.slice(0,100)).map((p,i)=>{
            ...
            const rank=i+1;
            const gpos=(p.gpos||p.pos)==="K"||(p.gpos||p.pos)==="P"||(p.gpos||p.pos)==="LS"?"K/P":((p.gpos||p.pos)==="C"||(p.gpos||p.pos)==="OG")?"IOL":(p.gpos||p.pos);
