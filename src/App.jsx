    ...
    <>
      ...
      <div style={{position:"fixed",top:0,right:0,bottom:0,width:Math.min(460,window.innerWidth-24),background:"#faf9f6",zIndex:201,boxShadow:"-8px 0 32px rgba(0,0,0,0.12)",transform:isOpen?"translateX(0)":"translateX(100%)",transition:"transform 0.3s cubic-bezier(0.16,1,0.3,1)",display:"flex",flexDirection:"column",overflowY:"auto"}}>
        ...
        <div style={{padding:"0 24px 16px"}}>
          ...
          {posTraits.map(trait=>{const val=traits[player.id]?.[trait]||50;const emoji=TRAIT_EMOJI[trait]||"";const hue=220+((100-val)/100)*60;const sat=50+val*.3;const light=35+((100-val)/100)*15;const barColor=`hsl(${hue},${sat}%,${light}%)`;return(
...
function DraftBoard({user,onSignOut}){
  ...
  if(phase==="home"||phase==="pick-position"){
    ...
    const hasStaleData=!hasBoardData&&Object.keys(ratings).length>0;
    ...
    {hasStaleData&&<div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:12,padding:"12px 16px",marginBottom:20,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <button onClick={()=>{setRatings({});setTraits({});setNotes({});setCompCount(0);}} style={{fontFamily:sans,fontSize:11,fontWeight:700,padding:"6px 14px",background:"#171717",color:"#faf9f6",border:"none",borderRadius:99,cursor:"pointer",flexShrink:0,marginLeft:12}}>reset</button>
    </div>}
