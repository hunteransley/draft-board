...
function PlayerProfile({player,traits,setTraits,allProspects,getGrade,onClose,onSelectPlayer}){
...
function DraftBoard({user,onSignOut}){
  ...
  useEffect(()=>{
    ...
    saveTimer.current=setTimeout(async()=>{
      ...
      setSaving(true);
      const boardData={ratings,traits,rankedGroups:[...rankedGroups],traitReviewedGroups:[...traitReviewedGroups],compCount};
  ...
  },[ratings,traits,rankedGroups,traitReviewedGroups,compCount,user.id,phase]);
  ...
  // === BIG BOARD ===
  if(phase==="board")return(<div><SaveBar/><div style={{paddingTop:32}}><BoardView getBoard={getBoard} getGrade={getGrade} rankedGroups={rankedGroups} setPhase={setPhase} setSelectedPlayer={setSelectedPlayer} setActivePos={setActivePos} traits={traits} compareA={compareA} compareB={compareB} setCompareA={setCompareA} setCompareB={setCompareB} setProfilePlayer={setProfilePlayer}/></div>{profilePlayer&&<PlayerProfile player={profilePlayer} traits={traits} setTraits={setTraits} allProspects={PROSPECTS} getGrade={getGrade} onClose={()=>setProfilePlayer(null)} onSelectPlayer={setProfilePlayer}/>}</div>);

  return(<>{profilePlayer&&<PlayerProfile player={profilePlayer} traits={traits} setTraits={setTraits} allProspects={PROSPECTS} getGrade={getGrade} onClose={()=>setProfilePlayer(null)} onSelectPlayer={setProfilePlayer}/>}</>);
}
...
function BoardView({getBoard,getGrade,rankedGroups,setPhase,setSelectedPlayer,setActivePos,traits,compareA,compareB,setCompareA,setCompareB,setProfilePlayer}){
  const[filterPos,setFilterPos]=useState(null);const board=getBoard();const display=filterPos?board.filter(p=>p.pos===filterPos):board;
